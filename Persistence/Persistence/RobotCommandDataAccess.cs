...
public static class RobotCommandDataAccess
{
    ...
    // GET single robot command by id
    ...
    // INSERT new robot command
    public static void AddRobotCommand(RobotCommand newCommand)
    {
        using var conn = new NpgsqlConnection(CONNECTION_STRING);
        conn.Open();
    ...
    public static bool UpdateRobotCommand(int id, RobotCommand updatedCommand)
    {
        ...
        return rows > 0; // returns true if something was updated
    }
    ...
    public static bool DeleteRobotCommand(int id)
    {
        ...
        cmd.Parameters.AddWithValue("@id", id);
        int rows = cmd.ExecuteNonQuery();
        return rows > 0; // returns true if something was deleted
    }
}